...
# SERAPHIM Installation Script - v1.9
...
# Adds Job Cancellation and Log Viewing capabilities.
...
VLLM_LOG_DIR_IN_INSTALLER="$SERAPHIM_DIR/seraphim_internal_logs" # This is more for backend logs, slurm logs go to SCRIPTS_DIR
...
echo "Starting SERAPHIM vLLM Deployment Setup (v1.9 with Job Cancel & Log View)..."
...
VLLM_LOG_DIR_PY = "{{VLLM_LOG_DIR_PLACEHOLDER}}" # Used for backend's own logs if any, not Slurm job logs.
...
class DeployedServiceInfo(BaseModel):
    ...
    slurm_error_file: Optional[str] = None # Added
...
def generate_sbatch_script_content(config: SlurmConfig, scripts_dir: str, conda_env_name: str) -> tuple[str, str, str, str]:
    ...
    # SCRIPTS_DIR_PY is the correct directory for Slurm logs
    slurm_out_file = os.path.join(SCRIPTS_DIR_PY, f"{safe_filename_job_name}_%j.out")
    slurm_err_file = os.path.join(SCRIPTS_DIR_PY, f"{safe_filename_job_name}_%j.err")
...
def parse_slurm_log_for_url(log_file_path: str, job_node: str, job_port: str) -> Optional[str]:
    ...
    try:
        with open(log_file_path, 'r', errors='ignore') as f:
            ...
            if job_node and job_port: # Fallback best guess
...
async def get_active_deployments():
    ...
    try:
        ...
        # Ensure squeue format includes job name, job ID, status, nodes, etc.
        ...
        for line in process.stdout.strip().split('\n'):
            ...
            try:
                ...
                job_name_squeue = parts[2].strip() # This is the SLURM job name
                ...
                node_list_val = parts[7].strip()   # Node where job is running

                if not job_name_squeue.startswith(JOB_NAME_PREFIX_FOR_SQ_PY):
                    continue # Filter for SERAPHIM jobs

                # Construct log file paths based on SCRIPTS_DIR_PY and job details
                # This assumes job_name_squeue from squeue is the one used in sbatch #SBATCH --job-name
                # The sbatch script uses config.job_name for #SBATCH --job-name=
                # And safe_filename_job_name for the log file name itself.
                # For robustness, we should try to match based on job_id and naming convention.
                # The 'safe_filename_job_name' is what's used in the log file.
                # This 'job_name_squeue' should be the same as 'config.job_name' from submission.
                
                # To construct log file names correctly, we need the 'safe' version of the job name.
                # The frontend submits `config.job_name`. The backend uses this for #SBATCH --job-name
                # and then creates `safe_filename_job_name` for the actual log files.
                # We need to use the *original* job name as submitted by user (which becomes job_name_squeue)
                # to form the 'safe' version.
                ...
                service_info = DeployedServiceInfo(
                    ...
                    slurm_output_file=slurm_out_file_path if os.path.exists(slurm_out_file_path) else None, # Store full path
                    slurm_error_file=slurm_err_file_path if os.path.exists(slurm_err_file_path) else None,   # Store full path
                    raw_squeue_line=line
                )

                if service_info.status == "R" and service_info.nodes:
                    # Extract port from job name (if present) for URL generation
                    job_port_from_name = "8000" # Default vLLM port
                    port_match_in_name = re.search(r"_p(\d{4,5})", job_name_squeue) # e.g., vllm_service_mymodel_p8001
                    ...
                    if service_info.slurm_output_file: # Only try to parse if file exists
                        service_info.service_url = parse_slurm_log_for_url(
                            service_info.slurm_output_file,
                            service_info.nodes.split(',')[0].split('(')[0].strip(), # Get first node
                            job_port_from_name
                        )
...
async def cancel_slurm_job_api(job_id: str):
    ...
    try:
        # Basic validation for job_id format (digits only)
        ...
        if process.returncode == 0:
            ...
            # squeue might return non-zero if job is already completed or gone, or if scancel had an issue.
            # Consider stderr as well.
            ...
            # It's often not a server error if scancel fails b/c job is gone, but we report it to user.
            raise HTTPException(status_code=400, detail=error_detail) # 400 or 500 depends on perspective
...
async def get_log_content_api(file_path: str = Query(..., description="Full path to the log file to read.")):
    ...
    # Security check: Ensure the requested path is within the allowed SCRIPTS_DIR_PY
    # This converts both to absolute paths and checks if file_path starts with SCRIPTS_DIR_PY
    ...
    if not os.path.exists(abs_file_path):
        logger.warning(f"Log file not found: {abs_file_path}")
        raise HTTPException(status_code=404, detail=f"Log file not found: {file_path}")
    
    if not os.path.isfile(abs_file_path): # Ensure it's a file, not a directory
    ...
    try:
        ...
        max_lines = 500 # Max lines to read
        max_bytes_head = 50 * 1024 # Max 50KB from start
        max_bytes_tail = 150 * 1024 # Max 150KB from end
        
        if file_size <= (max_bytes_head + max_bytes_tail): # If file is small enough, read all
             with open(abs_file_path, 'r', errors='ignore') as f:
                log_content = [line for line in f]
        else: # File is large, get head and tail
            with open(abs_file_path, 'r', errors='ignore') as f:
                # Read head
                bytes_read = 0
            ...
            with open(abs_file_path, 'rb') as f: # Open in binary for seeking from end
                f.seek(0, os.SEEK_END)
                end_pos = f.tell()
                seek_pos = max(0, end_pos - max_bytes_tail)
                f.seek(seek_pos)
                
                # Skip partial line if not at start of file
                if seek_pos > 0: 
                    f.readline() 

                # Read tail lines (decode from bytes)
                tail_lines_read = 0
                for line_bytes in f:
                    if tail_lines_read >= (max_lines // 2): break
                    try:
                        log_content.append(line_bytes.decode('utf-8', errors='ignore'))
                        tail_lines_read +=1
                    except UnicodeDecodeError:
                        log_content.append("[Undecodable line]\n")


        return {"log_content": "".join(log_content)}
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # VLLM_LOG_DIR_PY is for the backend's own separate logs, not Slurm job logs.
...
async function fetchLogContent(filePath, displayElementId) {
    ...
    displayElement.textContent = `🔄 Fetching ${filePath}...`;
    try {
        ...
        if (response.ok) {
            ...
            displayElement.textContent = `❌ Error fetching log (${response.status}): ${result.detail || response.statusText}`;
        }
    } catch (error) {
...
async function cancelJob(jobId) {
    const outputDiv = document.getElementById('output'); // For general messages
    ...
    try {
        ...
        refreshDeployedEndpoints(); // Refresh the list after attempting cancellation
    }
}
...
async function refreshDeployedEndpoints() {
    ...
    try {
        ...
        if (deployments.length === 0) {
            ...
            logOutputDisplay.textContent = "Select a running job to view its output log.";
            logErrorDisplay.textContent = "Select a running job to view its error log.";
        } else {
            let html = '<ul>';
            deployments.forEach(job => {
                html += `<li class="endpoint-item" 
                             data-jobid="${job.job_id}" 
                             data-outfile="${job.slurm_output_file || ''}" 
                             data-errfile="${job.slurm_error_file || ''}">
                            ...
                            ${job.slurm_error_file ? `<strong>Slurm Err:</strong> ${job.slurm_error_file}<br/>` : ''}
            ...
            listDiv.querySelectorAll('.endpoint-item').forEach(item => {
                item.addEventListener('click', function() {
                    ...
                    logOutputDisplay.textContent = "Select a running job to view its output log."; // Clear previous
                    logErrorDisplay.textContent = "Select a running job to view its error log.";  // Clear previous
                    
                    if (outFile) {
                       fetchLogContent(outFile, 'log-output-content');
                    } else {
                       logOutputDisplay.textContent = 'Output log file not available or job not running/found.';
                    }
                    if (errFile) {
                       fetchLogContent(errFile, 'log-error-content');
                    } else {
                       logErrorDisplay.textContent = 'Error log file not available or job not running/found.';
                    }
                });
            });
...
<head>
    ...
    <style>
        ...
        #output { margin-top: 20px; padding: 15px; background-color: #1c1c1e; border: 1px solid var(--border-color); border-radius: 6px; white-space: pre-wrap; word-wrap: break-word; font-size: 0.9em; max-height: 200px; overflow-y: auto; line-height: 1.6; font-family: 'Courier New', Courier, monospace; color: var(--text-color); }
        ...
        .endpoint-item { background-color: #3a3a3cdd; border: 1px solid #4a4a4e; padding: 12px; margin-bottom: 10px; border-radius: 6px; font-size: 0.85em; line-height: 1.5; box-shadow: inset 0 1px 2px rgba(0,0,0,0.1); cursor: pointer; transition: background-color 0.2s ease; }
        .endpoint-item:hover { background-color: #4f4f52; }
        .endpoint-item.selected { background-color: var(--primary-color); color: white; border-left: 5px solid var(--secondary-color); }
        ...
        .cancel-job-button {
            ...
            font-size: 0.8em;
            margin-top: 8px;
            margin-left: 0; /* Align with other content if needed */
        ...
        .log-display-area { display: flex; flex-wrap: wrap; gap: 20px; padding: 0 20px 20px 20px; max-width: 1300px; margin: 0 auto; width: 100%; box-sizing: border-box; }
        .log-box { flex: 1; min-width: 300px; background-color: var(--card-bg-color); padding: 20px; border-radius: 8px; border: 1px solid var(--border-color); box-shadow: 0 2px 8px rgba(0,0,0,0.15); }
        .log-box h3 { font-size: 1.2em; margin-bottom: 10px; color: var(--text-muted-color); }
        .log-box pre {
...
<body>
    ...
    <div class="log-display-area">
        <div class="log-box">
            <h3><span class="icon">📄</span> Output Log (.out) for Selected Job</h3>
            <pre id="log-output-content">Select a running job from the 'Active Deployments' list to view its output log.</pre>
        </div>
        <div class="log-box">
            <h3><span class="icon">⚠️</span> Error Log (.err) for Selected Job</h3>
            <pre id="log-error-content">Select a running job from the 'Active Deployments' list to view its error log.</pre>
        </div>
    </div>

    <div class="footer">✧ SERAPHIM CORE Interface v1.9 (Job Cancel & Log View) ✧ System Online ✧</div>
...
echo "======================================================================"
echo "✅ SERAPHIM Setup Complete! (v1.9 with Job Cancel & Log View)"
...
echo "======================================================================"
echo "🚨 Notes:"
echo "   - The user running the backend server needs sbatch and scancel access for deployments."
...
echo "   - Log viewing is restricted to files within '$SCRIPTS_DIR' for security."
echo "   - Large log files will be truncated (head and tail shown) in the UI."
echo "======================================================================"
exit 0
