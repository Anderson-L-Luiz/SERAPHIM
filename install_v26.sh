...
# Modifications based on user feedback for port handling and URL formatting.
...
def generate_sbatch_script_content(config: SlurmConfig, scripts_dir: str, conda_env_name: str) -> tuple[str, str, str, str]:
    ...
    # config.job_name here is already modified by deploy_vllm_service_api to include _p<port>
...
async def deploy_vllm_service_api(config: SlurmConfig, request: Request):
    logger.info(f"Deployment request for model: {config.selected_model}, Service Port: {config.service_port}, Max Model Len: {config.max_model_len}, Original Job Name from UI: {config.job_name}")

    # ---- START MODIFICATION FOR RELIABLE PORT IN JOB NAME ----
    job_name_from_input = config.job_name.strip()
    # Remove any existing _pXXXX or _pXXXXX suffix to prevent duplication like job_name_p1234_p5678
    base_job_name = re.sub(r"_p\d{4,5}$", "", job_name_from_input)
    # Construct the final job name with the correct port suffix
    final_slurm_job_name = f"{base_job_name}_p{config.service_port}"
    
    # Update the config object that will be passed to generate_sbatch_script_content
    original_config_job_name_for_logging = config.job_name # Keep original for logging if needed
    config.job_name = final_slurm_job_name # This is now the job name that will be submitted to Slurm
    logger.info(f"Derived Slurm Job Name: {config.job_name} (from UI input '{original_config_job_name_for_logging}' and service port {config.service_port})")
    # ---- END MODIFICATION ----
...
# ---- START MODIFICATION FOR URL FORMAT ----
def parse_slurm_log_for_url(log_file_path: str, node_ip_or_name: str, job_port: str) -> Optional[str]:
    url_base = None
    service_confirmed_running = False # Flag to check if Uvicorn line was found

    # Check for Uvicorn confirmation in the log (optional but good for UX)
        ...
        try:
            with open(log_file_path, 'r', errors='ignore') as f:
                for _ in range(200):  # Check first 200 lines
                    ...
                    if match:
                        log_ip_advertised, log_port_str = match.groups()
                        if log_port_str == job_port:
                            service_confirmed_running = True
                            logger.info(f"Uvicorn confirmed running on port {job_port} via log '{os.path.basename(log_file_path)}'. Host for URL: {node_ip_or_name}.")
                            break 
        except Exception as e:
            logger.warning(f"Could not read/parse log {log_file_path} for Uvicorn confirmation: {e}")

    # Always construct the URL if node_ip_or_name and job_port are available,
    # as the service might be running even if log check failed or line wasn't found.
    if node_ip_or_name and job_port:
        # node_ip_or_name is already the desired 10.16.246.X (if ki-g node) or fallback node name
        url_base = f"http://{node_ip_or_name}:{job_port}"
        if not service_confirmed_running:
             logger.info(f"Constructing service URL for host {node_ip_or_name}:{job_port}. Uvicorn startup line not confirmed in first 200 lines of log '{os.path.basename(log_file_path) if os.path.exists(log_file_path) else 'log N/A'}'.")
    
    if url_base:
        return f"{url_base}/docs"  # Append /docs

    logger.warning(f"Could not construct service URL for job (port: {job_port}, node: {node_ip_or_name}, log: {log_file_path}).")
    return None
# ---- END MODIFICATION FOR URL FORMAT ----
    ...
    try:
        ...
        for line in process.stdout.strip().split('\n'):
            ...
            try:
                ...
                # Use the squeue job name for log file naming consistency,
                # as this reflects what's actually in Slurm.
                safe_squeue_job_name_for_logs = "".join(c if c.isalnum() or c in ('_', '-') else '_' for c in job_name_squeue)
                out_file = os.path.join(SCRIPTS_DIR_PY, f"{safe_squeue_job_name_for_logs}_{job_id}.out")
                err_file = os.path.join(SCRIPTS_DIR_PY, f"{safe_squeue_job_name_for_logs}_{job_id}.err")
                
                node_ip, first_node_name, detected_port, service_url = None, None, None, None

                # ---- START MODIFICATION FOR DETECTED_PORT ----
                # Try to extract port from job name using the _pXXXXX pattern.
                # This pattern is now reliably added by deploy_vllm_service_api for SERAPHIM jobs.
                port_pattern_match = re.search(r"_p(\d{4,5})", job_name_squeue)
                if port_pattern_match:
                    detected_port = port_pattern_match.group(1)
                    if job_name_squeue.startswith(JOB_NAME_PREFIX_FOR_SQ_PY):
                        logger.info(f"Port {detected_port} extracted from SERAPHIM job name '{job_name_squeue}'.")
                    else:
                        logger.info(f"Port {detected_port} extracted from job name '{job_name_squeue}' (no SERAPHIM prefix, but matches port pattern).")
                else:
                    # No _p<port> pattern found in the job name.
                    if job_name_squeue.startswith(JOB_NAME_PREFIX_FOR_SQ_PY):
                        # This implies a SERAPHIM job (based on prefix) that somehow doesn't have the port pattern.
                        # Could be an older job or an issue.
                        logger.warning(f"SERAPHIM job '{job_name_squeue}' (prefix match) expected to have _pXXXXX port pattern but it was not found! Defaulting port to '8000'.")
                        detected_port = "8000" 
                    else:
                        # For non-SERAPHIM jobs (no prefix) and no port pattern, it's truly unknown.
                        logger.info(f"Job '{job_name_squeue}' (non-SERAPHIM or no port pattern) - port not found in name. Defaulting to '8000' for URL construction attempt.")
                        detected_port = "8000"
                # ---- END MODIFICATION FOR DETECTED_PORT ----
    ...
    try {
        ...
        if (deployments.length === 0) {
            ...
            deployments.forEach(job => {
                ...
                // Ensure /docs is part of the displayed URL if service_url is present
                if (job.service_url && !job.service_url.endsWith('/docs')) {
                    // This part of JS might be redundant if backend always adds /docs
                    // but good for robustness if backend might sometimes not.
                    // However, the primary fix is backend ensuring /docs is there.
                    // urlDisplay = job.service_url.replace(/^https?:\/\//, ''); // Keep it simple now
                }
                ...
                html += `<li class="endpoint-item" data-jobid="${job.job_id}" data-outfile="${outFile}" data-errfile="${errFile}">
                    ...
                    ${job.service_url ? `<br/><strong>Access:</strong> <a href="${job.service_url}" target="_blank" onclick="event.stopPropagation();">${urlDisplay}</a>` : (job.status === 'R' && job.node_ip && job.detected_port ? `<br/><em>Service on ${job.node_ip} (Port: ${job.detected_port || 'N/A'}) - Docs URL may appear after logs load.</em>` : '')}
...
async function handleDeployClick() {
    ...
    const slurmConfig = {
        ...
        job_name: document.getElementById('job-name').value, // Will be modified by backend to include _p<port>
...
<body>
        ...
        <div class="column deploy-column" id="deploy-form-column-wrapper">
            <div class="form-container"> 
                ...
                <div class="slurm-options">
                    ...
                    <label for="job-name">Job Name (e.g., {{JOB_NAME_PREFIX_FOR_SQ_PLACEHOLDER}}_model_pXXXX):</label>
    ...
    <div class="footer">✧ SERAPHIM CORE Interface v2.5 ✧ TDC AI | ANDERSON DE LIMA LUIZ ✧</div>
