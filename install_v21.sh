...
# SERAPHIM Installation Script - v2.1
# Implements 4-column layout, green log text, and auto-polling log updates.
...
echo "Starting SERAPHIM vLLM Deployment Setup (v2.1 - 4-Col Layout, Green Logs, Polling)..."
...
async def get_log_content_api(file_path: str = Query(..., description="Full path to the log file to read.")):
    ...
    try:
        ...
        if file_size <= (max_bytes_head + max_bytes_tail): # Small enough file
            ...
            # For tail, read last N KBs and take last N lines from that segment
            # This is an approximation for tail lines
            with open(abs_file_path, 'rb') as f: 
                f.seek(0, os.SEEK_END)
                # Consider files smaller than max_bytes_tail
                current_seek_offset = min(max_bytes_tail, f.tell())
                f.seek(-current_seek_offset, os.SEEK_END)
                
                if f.tell() > 0: # If not at the beginning of the file, discard partial line
                    f.readline() 
                ...
                # Get the actual last N lines from the read segment
...
const LOG_REFRESH_INTERVAL = 7000; // 7 seconds, adjust as needed
...
async function fetchLogContent(filePath, displayElementId) {
    ...
    // Avoid showing "Fetching..." if content is already there and only polling
    if (!displayElement.textContent || displayElement.textContent.startsWith('🔄 Fetching') || displayElement.textContent.startsWith('Log file path not available')) {
    ...
    try {
        ...
        // Only update if content has changed to avoid scroll jump if content is identical
        if (displayElement.textContent !== (result.log_content || '(empty log file)')) {
...
async function cancelJob(jobId) {
    ...
    try {
        ...
        if (currentSelectedJobDetails && currentSelectedJobDetails.jobId === jobId) {
            currentSelectedJobDetails = null; // Stop polling for cancelled job
        }
    } catch (error) {
...
async function refreshDeployedEndpoints(jobToSelect = null) {
    ...
    if (!jobToSelect && !currentSelectedJobDetails) { // Clear logs only if nothing was or will be selected
        logOutDisplay.textContent = "Select a job to view its output log.";
        logErrDisplay.textContent = "Select a job to view its error log.";
        currentSelectedJobDetails = null; // Ensure polling stops
    }
    ...
    try {
        ...
        if (deployments.length === 0) {
            listDiv.innerHTML = "<p>No active SERAPHIM vLLM deployments found.</p>";
            logOutDisplay.textContent = "No active deployments.";
            logErrDisplay.textContent = "No active deployments.";
            currentSelectedJobDetails = null; // Stop polling
        } else {
            let html = '<ul>';
            deployments.forEach(job => {
                ...
                html += `<li class="endpoint-item" data-jobid="${job.job_id}" data-outfile="${outFile}" data-errfile="${errFile}">
                    ...
                    <br/><button class="cancel-job-button" data-jobid="${job.job_id}" onclick="event.stopPropagation();">Cancel</button>
                 </li>`;
            });
            html += '</ul>';
            ...
            if (jobToSelect && jobToSelect.jobId) {
                ...
                if (itemToSelect) {
                    ...
                    newJobStillPresent = true;
                } else {
                    console.warn(`SERAPHIM_DEBUG: Auto-selected job ${jobToSelect.jobId} disappeared after refresh.`);
                }
            }
            // If the job we intended to auto-select is gone, or no job was intended for auto-selection,
            // and currentSelectedJobDetails points to a job that's no longer in the list, clear it.
            if (!newJobStillPresent && currentSelectedJobDetails) {
                const stillExists = deployments.some(d => d.job_id === currentSelectedJobDetails.jobId);
                if (!stillExists) {
                    console.log(`SERAPHIM_DEBUG: Previously selected job ${currentSelectedJobDetails.jobId} no longer exists. Clearing selection.`);
                    currentSelectedJobDetails = null;
                    logOutDisplay.textContent = "Selected job no longer active. Select another job.";
        ...
        logErrDisplay.textContent = "";
        currentSelectedJobDetails = null; // Stop polling on error
    } finally {
...
// Function to be called by setInterval for polling logs
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    await refreshDeployedEndpoints(); // Initial load of deployments

    setInterval(pollCurrentJobLogs, LOG_REFRESH_INTERVAL); // Start log polling
});
EOF_JS
...
<head>
    ...
    <style>
        ...
        html, body { height: 100%; margin: 0; padding: 0; overflow-x: hidden; } /* Prevent horizontal scroll on body */
        ...
        .page-content-wrapper { /* This is the main 4-column flex container */
            display: flex;
            flex-grow: 1; 
        ...
        .column { /* Common styling for all 4 columns */
            display: flex;
        ...
        /* Specific flex basis for each column type */
        .log-column { flex: 1 1 20%; min-width: 250px; } /* Logs take less initial space but can grow/shrink */
        .deploy-column { flex: 1.5 1 30%; min-width: 350px; } /* Deploy form slightly wider */
        .endpoints-column { flex: 1.2 1 25%; min-width: 300px; } /* Endpoints list */


        .column h3 {
            font-family: var(--font-heading); color: var(--secondary-color); 
            border-bottom: 1px solid var(--accent-color); padding-bottom: 8px; 
            margin-top: 0; margin-bottom: 15px; font-size: 1.2em; /* Slightly smaller h3 */
            letter-spacing: 1px; 
            display: flex; align-items: center; gap: 10px; flex-shrink: 0;
        }
        
        /* Styling for containers within columns if needed (e.g., form-container) */
        .form-container, .endpoints-container-inner {
             display: flex; flex-direction: column;
             overflow: hidden; 
             flex-grow: 1; /* Allow these to fill their parent column */
        }
        
        .log-column pre { /* Target pre tags for logs */
            background-color: #1a1a1c; 
            color: var(--log-text-color); /* USE GREEN FOR LOG TEXT */
            padding: 10px; /* Reduced padding */
            border-radius: 5px; font-family: 'Courier New', Courier, monospace;
            font-size: 0.8em; /* Slightly smaller log font */
        ...
        #model-search, select, input[type="text"], input[type="number"], input[type="email"], input[type="password"] { 
        ...
        #model-search:focus, select:focus, input:not(#model-search):focus { 
...
<body>
    ...
    <div class="page-content-wrapper">
        ...
        <div class="column deploy-column" id="deploy-form-column-wrapper">
            <div class="form-container"> <h3><span class="icon">⚙️</span> Deploy New vLLM Instance</h3>
        ...
        <div class="column endpoints-column" id="active-deployments-column-wrapper">
            <div class="endpoints-container-inner"> <h3><span class="icon">📡</span> Active Deployments</h3>
    ...
    <div class="footer">✧ SERAPHIM CORE Interface v2.1 (ANDERSON DE LIMA LUIZ) ✧</div>
...
# Start script content remains the same as v2.0, only re-generating to ensure it's part of the output block
...
# Stop script content remains the same as v2.0
...
echo "======================================================================"
echo "✅ SERAPHIM Setup Complete! (v2.1 - 4-Col Layout, Green Logs, Polling)"
...
echo "   - Log polling updates every ~7 seconds for the selected job."
echo "======================================================================"
exit 0
