...
install_package() {
  ...
  if ! command -v "$package" &> /dev/null; then
    ...
    if command -v apt-get &> /dev/null; then # Changed from apt to apt-get for wider compatibility
      ...
      echo "❌ Error: No common package manager (apt-get, yum, brew) found. Please install $package manually."
      exit 1
    fi
...
echo "🔥 Running CUM post-commit hook (commit details as blue paragraph)..."
...
if $is_initial_commit; then
    echo "📝 Initial commit detected. Listing all files in the commit."
    changed_files_str=$(git diff-tree --no-commit-id --name-only -r HEAD)
else
    echo "📝 Standard commit. Listing changed files against HEAD^."
...
if [ ${#changed_files_array[@]} -eq 0 ] && [ -n "$changed_files_str" ]; then
    echo "⚠️ Warning: Changed files string was not empty but parsed to zero files. Check for unusual filenames or only whitespace."
fi
...
for file_path in "${changed_files_array[@]}"; do
    ...
    if $is_initial_commit; then
        if git show "HEAD:$file_path" &>/dev/null; then
            ...
            echo "   ℹ️ File $file_path listed in initial commit tree but 'git show' failed (possibly removed before first commit or unusual state)."
    ...
    if [ -z "$file_diff_output_raw" ]; then
        echo "   ℹ️ No textual changes found for $file_path or content unavailable. Skipping API call."
    ...
    if [ "$sanitized_file_diff_len" -eq 0 ]; then
        echo "   ℹ️ No textual changes found for $file_path after sanitization. Skipping API call."
        ...
        current_offset=0
        echo "   ℹ️ Diff length for $file_path is $sanitized_file_diff_len bytes. Will be processed in $num_chunks chunk(s)."
        ...
        for (( chunk_num=1; chunk_num<=num_chunks; chunk_num++ )); do
            ...
            echo "   📞 Contacting LLM API for $file_path (Part $chunk_num/$num_chunks, ${#file_diff_chunk} chars)..."
            ...
            if [ $? -ne 0 ]; then
                echo "   ❌ Hook Error: Failed to construct JSON payload for $file_path (Part $chunk_num/$num_chunks)." >&2
                ...
                if [ $curl_exit_status_file_chunk -ne 0 ]; then
                    echo "   ❌ Hook Warning: LLM API request failed for $file_path (Part $chunk_num/$num_chunks) (curl code: $curl_exit_status_file_chunk)." >&2
                    ...
                    if echo "$api_response_file_chunk" | jq -e '.choices[0].text' > /dev/null 2>&1; then
                        ...
                        echo -e "   ✅ Summary for $file_path (Part $chunk_num/$num_chunks) received."
                    else
                        echo "   ❌ Hook Warning: Failed to extract summary text for $file_path (Part $chunk_num/$num_chunks) from response." >&2
...
if [ $awk_status -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
        echo "✅ Summaries for commit $commit_short_hash appended to $TEX_FILE_PATH"
...
echo "✅ CUM post-commit hook (commit details as blue paragraph) finished for $commit_short_hash."
exit 0
HOOK_SCRIPT_EOF
...
echo "ℹ️  The hook is now configured to send requests to your vLLM API using the /v1/completions endpoint at: $VLLM_API_URL"
...
echo "‼️ IMPORTANT: Ensure your vLLM server at $VLLM_API_URL is running and"
