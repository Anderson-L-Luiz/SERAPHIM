...
# SERAPHIM Installation Script - v2.3
...
echo "Starting SERAPHIM vLLM Deployment Setup (v2.3 - Matrix Theme & Enhancements)..."
    ...
    try:
        with open(log_file_path, 'r', errors='ignore') as f:
            ...
            if host_for_url and job_port: 
                return f"http://{host_for_url}:{job_port}/docs (best guess, check log for actual URL)"
...
async function fetchLogContent(filePath, displayElementId) {
    // ... (fetchLogContent logic from v2.2, minor tweak for scroll) ...
    ...
    try {
        ...
        if (displayElement.textContent !== newContent) {
             ...
             // Smart scroll: only scroll to bottom if user hasn't scrolled up, or on initial load
...
<head>
    ...
    <style>
        ...
        .cancel-job-button {
            ...
            margin-top: 4px; width: auto; display: inline-block; float: right;
...
<body>
    ...
    <div class="footer">✧ SERAPHIM CORE Interface v2.3 (Matrix Edition) ✧ TDC AI | ANDERSON LUIZ ✧</div>
...
echo "======================================================================"
echo "✅ SERAPHIM Setup Complete! (v2.3 - Matrix Edition)"
